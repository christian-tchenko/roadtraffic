...
class Window:
    ...
    def create_windows(self):
        ...
        with dpg.window(
            tag="ControlsWindow",
            label="Tableau Contrôle",
            no_close=True,
            no_collapse=True,
            no_resize=True,
            no_move=True
        ):
            with dpg.collapsing_header(label="Contrôle de la simulation", default_open=True):

                with dpg.group(horizontal=True):
                    dpg.add_button(label="Lancer", tag="RunStopButton", callback=self.toggle)
                    #dpg.add_button(label="Next frame", callback=self.simulation.update)

                dpg.add_slider_int(tag="SpeedInput", label="Vitesse", min_value=1, max_value=100,default_value=1, callback=self.set_speed)
            
            with dpg.collapsing_header(label="État de la simulation", default_open=True):
            ...
            with dpg.collapsing_header(label="Contrôle de la caméra", default_open=True):
                ...
                with dpg.group():
                    dpg.add_slider_float(tag="OffsetXSlider", label="Décalage - X", min_value=-100, max_value=100, default_value=self.offset[0], callback=self.set_offset_zoom)
                    dpg.add_slider_float(tag="OffsetYSlider", label="Décalage - Y", min_value=-100, max_value=100, default_value=self.offset[1], callback=self.set_offset_zoom)
    ...
    def update_panels(self):
        ...
        if self.is_running:
            ...
            dpg.set_value("StatusText", "Arrêté")
    ...
    def draw_bg(self, color=(0, 0, 0)): #(250, 250, 250)
        dpg.draw_rectangle(
            (-10, -10),
    ...
    def draw_signal(self,  color=(0, 255, 0)):
        for sig in self.simulation.signals:
            dpg.draw_polyline(sig.points, color=color, thickness=3.5*self.zoom, parent="Canvas")
            
    def draw_signal_red(self,  color=(255, 0, 0)):
        for sigr in self.simulation.signalsred:
            dpg.draw_polyline(sigr.points, color=color, thickness=3.5*self.zoom, parent="Canvas")
    ...
    def run(self):
        ...
        dpg.set_item_label("RunStopButton", "Arrêter")
        dpg.bind_item_theme("RunStopButton", "StopButtonTheme")

    def stop(self):
        self.is_running = False
        dpg.set_item_label("RunStopButton", "Lancer")
